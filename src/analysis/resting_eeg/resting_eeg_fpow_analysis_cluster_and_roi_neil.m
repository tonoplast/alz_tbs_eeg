...
outPath = [inPath, filesep 'results' filesep 'neil' filesep];
...
for i = 1:numel(data_groups)
    ...
    file_name = fullfile(inPath, ['power_', current_group, '_neil.mat']);
...
% drop-outs)
[data_filtered.eyesclosed_BL, data_filtered.eyesclosed_END] = filterDatasets(data.eyesclosed_BL, data.eyesclosed_END, true);
[data_filtered.eyesopen_BL, data_filtered.eyesopen_END] = filterDatasets(data.eyesopen_BL, data.eyesopen_END, true);

% Split into EO and EC
data_ec = splitAndFilterDatasetsByLabels(data_filtered.eyesclosed_BL, data_filtered.eyesclosed_END, active_group, sham_group, true);
data_eo = splitAndFilterDatasetsByLabels(data_filtered.eyesopen_BL, data_filtered.eyesopen_END, active_group, sham_group, true);
