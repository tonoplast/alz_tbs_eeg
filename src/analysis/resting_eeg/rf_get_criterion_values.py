# -*- coding: utf-8 -*-
"""
Created on Thu Nov  9 18:01:13 2023
    ...
    for target_variable in target_variables:
        for fn_suffix, x_col_list in zip(fn_suffixes, x_col_lists):
            ...
            for name, model in base_models.items():
                ...
                # Initialize lists to store all FPRs, TPRs, and thresholds
                ...
                for train_index, test_index in sss.split(X_reduced, y):
                    ...
                    # Append to the lists initialized before the loop
                    all_fpr.extend(fpr)
                    all_tpr.extend(tpr)
            ...
            # After the loop, create the criterion table DataFrame for the current model
            criterion_table = pd.DataFrame({
                      'False Positive Rate': all_fpr,
                      'True Positive Rate': all_tpr,
                      'Thresholds': all_thresholds
                  }).drop_duplicates().sort_values('Thresholds', ascending=False).reset_index(drop=True)
